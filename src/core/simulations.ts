...
export const SIMULATION_INFO: Record<SimulationType, SimulationInfo> = {
	...
	[SimulationType.DayAndNight]: {
		...
		wikiUrl: "https://en.wikipedia.org/wiki/Day_and_Night_(cellular_automaton)",
		category: "2D",
	},
	[SimulationType.Maze]: {
		label: "Maze",
...
export function updateConway(grid: Grid): { grid: Grid } {
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			neighbourCounts[y][x] = countNeighbours2D(grid, x, y, (s) => s !== 0);
		}
	}
...
function compute1DRow(above: number[], rule: number, width: number): number[] {
	...
	for (let x = 0; x < width; x++) {
		...
		const pattern =
			(left !== 0 ? 4 : 0) | (center !== 0 ? 2 : 0) | (right !== 0 ? 1 : 0);
